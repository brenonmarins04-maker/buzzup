...
export default function ProjectsPage() {
  const { projects, teams, tasks, posts } = useData();
          ...
          return (
            <div key={project.id} onClick={() => setModal({ open: true, project })} className="bg-card border border-border rounded-lg p-5 hover:shadow-md transition-all cursor-pointer flex flex-col gap-4">
              ...
              <div className="flex items-center gap-2">
                <span className={`h-2 w-2 rounded-full bg-team-${team?.color?.replace("team-", "")}`} />
              ...
              <div>
                ...
                <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                  <div className="h-full bg-foreground rounded-full transition-all" style={{ width: `${projectTasks.length > 0 ? (completedTasks / projectTasks.length) * 100 : 0}%` }} />
                </div>
              </div>
            </div>
          );
