...
// Delay helpers
const PANEL_DURATION = 1200; // ms — how long the 50vw→380px slide takes
...
export default function WelcomePage() {
  ...
  // ── Animation states ──────────────────────────────────────────────────────
  // panelSlid: true = panel is at its final width (380px); false = still at 50vw
  const [panelSlid, setPanelSlid] = useState(fromLogin);
  // contentIn: true = right-side content is visible
  ...
  useEffect(() => {
    if (fromLogin) {
      // Panel is already at the right size — just fade content in quickly
      const t = setTimeout(() => setContentIn(true), 60);
      return () => clearTimeout(t);
    } else {
      // Animate panel first, then content after it lands
      const raf = requestAnimationFrame(() =>
        requestAnimationFrame(() => setPanelSlid(true))
      );
      const t = setTimeout(() => setContentIn(true), PANEL_DURATION * 0.72);
  ...
  // Stagger: each item gets progressively more delay
  const stagger = (i: number, baseMs = 0) =>
    `opacity 0.28s ease-out ${baseMs + i * 65}ms, transform 0.28s ease-out ${baseMs + i * 65}ms`;

  const fadeUp = (i: number, baseMs = 0): React.CSSProperties => ({
    opacity: contentIn ? 1 : 0,
    transform: contentIn ? "translateY(0)" : "translateY(14px)",
    transition: stagger(i, baseMs),
  });
  ...
  return (
    <div className="h-screen overflow-hidden flex bg-[#f7f7f5]">
      ...
      <aside
        ...
        style={{
          width: panelSlid ? 380 : "50vw",
          transition: fromLogin ? "none" : `width ${PANEL_DURATION}ms cubic-bezier(0.25, 0.46, 0.45, 0.94)`,
        }}
      >
        {/* Logo */}
        ...
        >
          <p className="text-white/50 text-sm font-semibold uppercase tracking-widest mb-3 whitespace-nowrap">
            Bem-vindo de volta
          </p>
          ...
          {user?.email && (
            <p className="text-white/55 mt-3 text-sm overflow-hidden text-ellipsis whitespace-nowrap">{user.email}</p>
          )}
        ...
        <div className="flex items-center justify-between shrink-0">
          ...
          <button onClick={() => signOut()} className="flex items-center gap-1.5 text-xs text-white/40 hover:text-white/70 transition-colors">
        ...
        <div className="px-8 py-8 lg:py-10 h-full flex flex-col">
          ...
          {mode === "hub" && (
            <div className="space-y-8 flex-1">
              ...
              {/* Ações — stagger após os workspace cards */}
              <div className="grid sm:grid-cols-2 gap-3">
                <button
